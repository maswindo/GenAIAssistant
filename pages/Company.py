...
def load_company_data():
    ...
    if company_name:
        ...
        if st.session_state["glassdoor_data"] is None:
            ...
            if st.session_state["glassdoor_data"]:
...
#-----------------------------------Chatbot----------------------------------------------------------
    ...
    with st.expander("Chat with the Assistant", expanded=False):
        ...
        # Display chat messages within a scrollable container
        ...
        # Input for user query placed below the chat messages
...
#-----------------------------------Glassdoor Reviews----------------------------------------------------------
    ...
    if summary_data:
        ...
        st.write("#### Glassdoor Reviews (Last 3 Months)")
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("##### Positive Highlights ")
            st.write(summary_data["pro_short_summary"])
        with col2:
            st.markdown("##### Challenges and Concerns ")
        ...
        with col2:
            ...
            if st.session_state["filtered_reviews"]:
                ...
                cons_summary = summarize_reviews(filtered_reviews["cons"], "cons")
                        # Display summaries
                st.subheader("Summary of Filtered Reviews")
                st.write("**Pro**")
                st.write(pros_summary)
                st.write("**Con:**")
                st.write(cons_summary)
        
    st.write(f"#### Specialities and Affiliated Companies") 
#---------------------------------Specialities------------------------------------------------------------------
    col1, col2 = st.columns(2)
    with col1:
        with st.expander(f"#### Specialties", expanded=False):
            ...
            if specialties:
                ...
                st.write("No specilaities")
#-----------------------------------Affiliated Companies----------------------------------------------------------

    affiliated_companies = company_linkedin_data.get("affiliated_companies", [])
    with col2:
        with st.expander(f"#### Affiliated Companies", expanded=False):
            if affiliated_companies:
                ...
                st.write("No Affiliated Companies")
